...
export default function AdminOrdersPage() {
  ...
  const handleDelete = async (id: string, orderId: string) => {
    const confirmed = window.confirm(
      `Yakin mau hapus pesanan ${orderId}? Data yang udah dihapus gak bisa dibalikin lagi.`,
    );
    if (!confirmed) return;
    ...
    if (!error) {
      ...
      window.alert("Gagal hapus pesanan, coba lagi ya.");
    }
    setDeletingId(null);
  };
      ...
      <div className="relative mx-auto w-[92%] max-w-5xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/admin/finance"
              className="neo-shadow-sm flex items-center gap-2 rounded-full border-[3px] border-navy bg-sunny px-4 py-2 font-body text-sm font-semibold text-navy"
            >
              <BarChart3 size={16} />
              Keuangan
            </Link>
          ...
          {loadingOrders ? (
            ...
            filteredOrders.map((order) => (
              ...
              >
                ...
                {/* Ganti status + hapus (khusus rejected) */}
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    {updatingId === order.id && (
                      <Loader2
                        size={14}
                        className="animate-spin text-navy/40"
                      />
                    )}
                  </div>
                  ...
                  {order.status === "rejected" && (
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-full border-[3px] border-coral bg-white px-3 py-1.5 font-body text-xs font-semibold text-coral transition-colors hover:bg-coral hover:text-white disabled:opacity-50"
                    >
                      {deletingId === order.id ? (
